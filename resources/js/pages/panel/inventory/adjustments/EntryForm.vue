...
async function handleSubmit() {
  if (!form.warehouse_id || form.items.some(i => !i.product_id || i.quantity_after <= 0)) {
    toast({
      title: 'Validación',
      description: 'Completa todos los campos. La cantidad debe ser mayor a 0.',
      variant: 'destructive'
    });
    return;
  }
  ...
  try {
    ...
    toast({ title: 'Éxito', description: 'Entrada de almacén registrada.' });
...
<template>
  <div class="flex flex-col gap-6 max-w-6xl mx-auto">
    <div class="flex items-center gap-4">
      ...
      <div>
        <h1 class="text-3xl font-bold tracking-tight">Nueva Entrada</h1>
        <p class="text-muted-foreground">Registra el ingreso de mercancía al almacén.</p>
      </div>
    </div>
