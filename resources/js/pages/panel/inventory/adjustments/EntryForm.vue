...
async function handleSubmit() {
  if (!form.warehouse_id || form.items.some(i => !i.product_id || i.quantity_after <= 0)) {
    toast({
      title: t('common.validation_error'),
      description: t('adjustments.validation_msg_positive'),
      variant: 'destructive'
    });
    return;
  }
  ...
  try {
    ...
    toast({ title: t('common.success'), description: t('adjustments.entry_success') });
    router.push('/panel/inventory/adjustments');
  } catch (error) {
    toast({
      title: t('common.error'),
      description: t('adjustments.entry_error'),
      variant: 'destructive'
    });
  } finally {
...
<template>
  <div class="flex flex-col gap-6 max-w-6xl mx-auto">
    <div class="flex items-center gap-4">
      ...
      <div>
        <h1 class="text-3xl font-bold tracking-tight">{{ t('adjustments.new_entry') }}</h1>
        <p class="text-muted-foreground">{{ t('adjustments.subtitle_entry') }}</p>
      </div>
    </div>
        ...
        >
          ...
          <div class="md:col-span-2 space-y-2 relative">
            ...
            <div class="absolute -bottom-5 left-0 text-[10px] text-muted-foreground whitespace-nowrap">
              {{ t('adjustments.in_stock') }}: <span class="font-medium text-primary">{{ item.available }}</span>
            </div>
          </div>
    ...
    <div class="flex justify-end">
        ...
        >
            <Save class="mr-2" />
            {{ isSubmitting ? t('common.saving') : t('adjustments.submit_entry') }}
        </Button>
    </div>
  </div>
</template>
